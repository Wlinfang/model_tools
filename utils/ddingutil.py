...
class DingDing:
    ...
    def send_markdown(self, title, content, phones=[]):
        """
        :param title:
        :param content: markdown 格式 https://open.dingtalk.com/document/orgapp/custom-robot-access
        :param phones:
        :return:
        """
        params = dict()
        params['msgtype'] = 'markdown'
        params['markdown'] = {'title': title, 'text': content}
        if not phones:
            params['at'] = {'atMobiles': phones}
        else:
            params['at'] = {'isAtAll': False}
        ...
        if res.get('errcode') == 0:
            ...
            logging.error('dingding send msg error %s', res)
            return False
